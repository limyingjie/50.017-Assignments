...
// Globals here.
float hArg = 0.04f;
enum SYSTEMS {SIMPLE,PENDULUM,CLOTH};
SYSTEMS currentEnumSystem;
...
namespace
{
  ...
  void initSystem(int argc, char * argv[])
  {
	...
	if (argc >= 3) {
		hArg = atof(argv[2]);
		cerr << "Step size: " << hArg << endl;
	}
	cerr << "Press t to switch modes" << endl;
  }
  ...
  void drawSystem()
  {
    ...
    void keyboardFunc( unsigned char key, int x, int y )
    {
		...
		case 't':
		{
			switch (currentEnumSystem) 
			{
			case SIMPLE: 
			{
				currentEnumSystem = PENDULUM;
				system = new PendulumSystem(4);
				break;
			}
			case PENDULUM:
			{
				currentEnumSystem = CLOTH;
				system = new ClothSystem(20,10,0.1f);
				break;
			}
			default:
				currentEnumSystem = SIMPLE;
		...
		case 'w': 
		{
			if (currentEnumSystem == CLOTH)((ClothSystem *)system)->toggleWireframe();
			break;
		}
        default:
